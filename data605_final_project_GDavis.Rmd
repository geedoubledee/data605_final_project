...
The p-value for the Chi Square test is large: 0.1345. So we reject the first null hypothesis that the variables are independent. 
...
The p-value for the Fisher's Exact test is also large: 0.1369. So we reject the second null hypothesis that the variables are independent. 
...
```{r }
lm_bc <- boxcox(train_df_model_sub_lm_update)
